...
@pytest.mark.asyncio
async def test_agent_slot_filling_v2_decline_confirmation_does_not_book(
    ...
    async def post_json(url: str, payload: dict[str, Any], headers: dict[str, str] | None = None) -> dict[str, Any]:
        ...
        if tool_name == "get_case_status":
            ...
            # Any booking attempt should fail this test.
            assert tool_name not in {"confirm_services", "store_service_order"}
            result_obj = {"ok": True, "tool_args": tool_args}
    ...
    async with AgentSession() as session:
        ...
        start = time.monotonic()
        while not any("No problem" in msg for msg in assistant_messages) and time.monotonic() - start < 1.0:
            await asyncio.sleep(0.01)

    assert "store_service_order" not in calls
    assert any("No problem" in msg for msg in assistant_messages)
