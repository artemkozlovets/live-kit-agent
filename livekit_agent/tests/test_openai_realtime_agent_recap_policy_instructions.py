...
@pytest.mark.asyncio
async def test_openai_realtime_agent_injects_pre_booking_recap_policy_instructions() -> None:
    backend = _Backend({"current_phase": "customer_intake"})
    ...
    async with AgentSession() as session:
        def fake_generate_reply(*, instructions: str | None = None, **_kwargs: Any) -> None:
            ...
            assert "Recap policy (pre-booking)" in instructions
...
@pytest.mark.asyncio
async def test_openai_realtime_agent_injects_booking_recap_policy_instructions() -> None:
    backend = _Backend({"current_phase": "booking"})
    agent = OpenAIRealtimeAgent(backend_client=backend, call_id_fallback="room-test")  # type: ignore[arg-type]

    async with AgentSession() as session:
        def fake_generate_reply(*, instructions: str | None = None, **_kwargs: Any) -> None:
            assert isinstance(instructions, str)
            assert "Recap policy (booking)" in instructions
